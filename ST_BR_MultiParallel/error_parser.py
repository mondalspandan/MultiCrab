...
for ifile in filelist:
    line = subprocess.check_output(['tail', '-3', os.path.join(folder,ifile)])
    if line.split('\n')[0].strip() != "TFile::Append:0: RuntimeWarning: Replacing existing TH1: h_total_mcweight (Potential memory leak).":
        n=2
        while line.strip() == "":
            n+=1
